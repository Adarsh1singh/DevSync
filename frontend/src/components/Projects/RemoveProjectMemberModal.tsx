...
const RemoveProjectMemberModal: React.FC<RemoveProjectMemberModalProps> = ({ 
  ...
  const handleRemove = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/projects/${member.projectId}/members/${member.id}`, {
        method: 'DELETE',
        headers: {
      ...
      toast.success(`${member.user.firstName} ${member.user.lastName} removed from ${projectName}`);
      onClose();
    } catch (error: any) {
          ...
          <div className="mb-6">
            <div className="flex items-center space-x-4 p-4 bg-gray-50 rounded-lg mb-4">
              ...
              <div className="flex-1">
                ...
                <div className={`inline-flex items-center space-x-1 px-2 py-1 rounded-full text-xs font-medium mt-1 ${getRoleBadge(member.role || 'MEMBER')}`}>
            ...
            <p className="text-gray-700 mb-4">
              Are you sure you want to remove <strong>{member.user.firstName} {member.user.lastName}</strong> from <strong>{projectName}</strong>?
            </p>
                ...
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    <h4 className="text-sm font-medium text-yellow-800 mb-1">
                      Warning: Removing Project Admin
                    </h4>
                    <p className="text-sm text-yellow-700">
                      This member is a project admin. Removing them will revoke their administrative privileges.
                      Make sure another admin is available to manage the project.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
              ...
              <ul className="text-sm text-gray-700 space-y-1">
                <li>• Remove the member from this project</li>
                <li>• Revoke access to project tasks and data</li>
                <li>• Unassign them from project tasks</li>
                <li>• Remove their project role and permissions</li>
              </ul>
            </div>
          </div>
